...
console.log('Theory     :', theory.length, '| malformed title/answer:', malformedTheory.length, '| exact duplicates:', duplicateTheory.length);
console.log('Quiz       :', quiz.length, '| explanation <=15 words:', shortQuiz.length, '| correct is longest:', weakLengthQuiz.length);
console.log('Checklist  :', checklist.length, '| junk:', badChecklist.length, '| exact duplicates:', duplicateChecklist.length);
console.log('Algorithms :', algo.length, '| incomplete:', badAlgo.length);
...
if (badChecklist.length || badAlgo.length) process.exitCode = 1;
